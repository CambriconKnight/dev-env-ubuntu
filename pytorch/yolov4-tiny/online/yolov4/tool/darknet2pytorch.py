...
class Mish(torch.nn.Module):
    ...
    def forward(self, x):
        if x.device.type != 'mlu':
            x = x * torch.tanh(torch.log(torch.exp(x) + 1))
        else:
            x = torch.ops.torch_mlu.mish(x)
        return x
...
class Upsample_expand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=[stride * H, stride * W], mode="nearest")
        return x
...
class Darknet(nn.Module):
    def __init__(self, cfgfile):
        ...
        self.seen = 0
        self.yolov_ver = 'yolov4' if 'tiny' not in cfgfile else 'yolov4-tiny'
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            print("Use MLU Yolov4_detection op !")
            # Extract anchors
            anchors = [12, 16, 19, 36, 40, 28, 36, 75, 76, 55, 72, 146, 142, 110, 192, 243, 459, 401]
            if self.yolov_ver == 'yolov4-tiny':
            ...
            nms_thres = 0.6
            maxBoxNum = 1024
            num_anchors = 18 if self.yolov_ver == 'yolov4' else 12
            scale_x_y = 1.05
            beta = 0.6
    ...
    #             print('unknown type %s' % (block['type']))
    #     fp.close()
def yolov4(pretrained=False, quantize=False, yolov4_version='yolov4'):
    cfg_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../model/' + yolov4_version + '.cfg')
    print("\033[1;31;40mUse {0} .\n cfgfile is {1} !!! \033[0m".format(yolov4_version, cfg_path))
    ...
    if pretrained:
        ...
        pretrained_weight = os.path.join(TORCH_HOME, 'checkpoints/' + yolov4_version + '.pth')
