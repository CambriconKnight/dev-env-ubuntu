...
def yolo_forward(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
    ...
    grid_x = torch.linspace(0, W - 1, W).reshape(1, 1, 1, W).repeat(1, 1, H, 1).to(device).float()
    grid_y = torch.linspace(0, H - 1, H).reshape(1, 1, H, 1).repeat(1, 1, 1, W).to(device).float()
    ...
    for i in range(num_anchors):
        ii = i * 2
        # Shape: [batch, 1, H, W]
        # grid_x_tensor = torch.tensor(grid_x, dtype=torch.float32).to(device)
        bx = bxy[:, ii : ii + 1] + grid_x
        # bx = bxy[:, ii] + torch.tensor(grid_x, device=device, dtype=torch.float32) # grid_x.to(device=device, dtype=torch.float32)
        # Shape: [batch, 1, H, W]
        # grid_y_tensor = torch.tensor(grid_y, dtype=torch.float32).to(device)
        by = bxy[:, ii + 1 : ii + 2] + grid_y
    ...
    boxes = torch.cat((bx1, by1, bx2, by2), dim=2).view(batch, num_anchors * H * W, 4)
...
def yolo_forward_dynamic(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
    ...
    boxes = torch.cat((bx1, by1, bx2, by2), dim=2).view(output.size(0), num_anchors * output.size(2) * output.size(3), 4)
...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        # print('output shape:{}, anchor_mask:{}'.format(output.shape, self.anchor_mask))
        # print('anchors:{}, stride:{}'.format(self.anchors, self.stride))
        masked_anchors = []
        for m in self.anchor_mask:
            masked_anchors += self.anchors[m * self.anchor_step:(m + 1) * self.anchor_step]
            # print('mask_anchor:{}, stride:{}, masked anchors:{}'.format(self.anchor_step, self.stride, masked_anchors))
        masked_anchors = [anchor / self.stride for anchor in masked_anchors]
        # print('thresh:{}, num_calss:{}, mask_anchors:{}, len(anchor_mask):{}'.
                # format(self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask)))
        return yolo_forward_dynamic(output, self.thresh, self.num_classes, masked_anchors,
