...
def quantification(args):
    ...
    with torch.no_grad():
        ...
        model = yolov4(pretrained=True, quantize=False,cfgfile=args.cfgfile)
        ...
        input_w = model.width
        print("input_w:{}input_h:{}".format(input_w,input_h))
        with open(args.image_list,"r") as f:
            image_list=[x.strip() for x in f.readlines()]

        #准备数据
        for path in image_list[0:iteration]:
            img = cv2.imread(path)
...
def calc_coco(args):
    ...
    with torch.no_grad():
        #model = yolov4(pretrained=True, quantize=bool(args.quantized_mode), args.cfgfile)
        model = yolov4(pretrained=True, quantize=bool(args.quantized_mode),cfgfile=args.cfgfile)
        model.to(device)
...
def get_args():
    ...
    parser.add_argument('-img_num', type=int, default=16, help="number of image")
    ...
    parser.add_argument('-image_list', default='./image_list', type=str, help='image_list')
    parser.add_argument('-iteration', type=int, default=16, help=" number of images used for quantization")
