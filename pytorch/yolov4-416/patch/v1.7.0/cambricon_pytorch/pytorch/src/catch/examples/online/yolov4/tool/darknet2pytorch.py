...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            maxBoxNum = 1024
            num_anchors = 18
            detect_out = torch.ops.torch_mlu.yolov3_detection_output(out_boxes[0],
                                                                     out_boxes[1],
                                                                     out_boxes[2],
                                                                     tuple(anchors),
                                                                     num_classes,
    ...
    #             print('unknown type %s' % (block['type']))
    #     fp.close()
def yolov4(pretrained=False, quantize=False):
    cfg_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../model/yolov4.cfg')
    ...
    if pretrained:
        ...
        pretrained_weight = os.path.join(TORCH_HOME, 'checkpoints/yolov4.pth')
