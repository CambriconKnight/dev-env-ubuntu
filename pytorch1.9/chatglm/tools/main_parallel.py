    ...
    if training_args.do_train:
        ...
        for k, v in device_map_dict.items():
            if k == 'transformer.word_embeddings':
                model.transformer.word_embeddings = model.transformer.word_embeddings.to(f'mlu:{v}')
            if k.find("transformer.layers") != -1:
                sub_value = int(k.replace("transformer.layers.", ""))
                model.transformer.layers[sub_value] = model.transformer.layers[sub_value].to(f'mlu:{v}')

            if k == "transformer.final_layernorm":
                model.transformer.final_layernorm = model.transformer.final_layernorm.to(f'mlu:{v}')

            if k == "transformer.prefix_encoder":
                model.transformer.prefix_encoder = model.transformer.prefix_encoder.to(f'mlu:{v}')
            if k == "lm_head":
                model.lm_head = model.lm_head.to(f'mlu:{v}')
